...
const regex = /const handleDeleteActiveEntry = async \(\) => \{[\s\S]*?deletedIdsRef\.current\.delete\(targetId\);\s*\}\s*\}\s*\};/;

const exactLogic = `const handleDeleteActiveEntry = async () => {
    if (!activeId) return;
...
console.log("Patched Active delete exact logic");
